...
// query data menu makanan berdasarkan id
$mkn = query("SELECT * FROM beasiswa WHERE id_beasiswa = $id_beasiswa")[0];
...
<html lang="en">
    ...
    <!-- ubah beasiswa -->
	<section id="ubahbeasiswa">
		<div class="container">
			<div class="row justify-content-center">
				<div class="col-md-8 text-center">
					<h2 class="fw-bold color-acsent2">Edit Formulir beasiswa</h2>
					<h2 class="fw-bold color-acsent">MajuBersama</h2>
				</div>
			</div>
			<div class="row justify-content-center">
                <div class="col-8">
                    <div class="formlomba mt-5">
                        <form action="" method="POST" enctype="multipart/form-data">
						<input type="hidden" name="id_beasiswa" value="<?= $mkn["id_beasiswa"]; ?>">
		                <input type="hidden" name="gambarLama" value="<?= $mkn["gambar_beasiswa"]; ?>">
                        <div class="mb-3">
                                <label for="exampleInputNo" class="form-label">Judul beasiswa</label>
                                <input type="text" class="form-control" name="judul_beasiswa" id="judul_beasiswa" aria-describedby="emailHelp" value="<?= $mkn ["judul_beasiswa"]; ?>">
                            </div>
                        <div class="mb-3">
                                <label for="exampleInputNo" class="form-label">Kategori beasiswa</label>
                                <input type="text" class="form-control" name="kategori_pendaftar" id="kategori_beasiswa" aria-describedby="emailHelp" value="<?= $mkn ["kategori_pendaftar"]; ?>">
                            </div>
                        <div class="mb-3">
                                <label for="exampleInputNo" class="form-label">Deskripsi beasiswa</label>
                                <input type="text" class="form-control" name="deskripsi_beasiswa" id="deskripsi_beasiswa" aria-describedby="emailHelp" value="<?= $mkn ["deskripsi_beasiswa"]; ?>">
                            </div>
                        <div class="mb-3">
                                <label for="exampleInputNo" class="form-label" for="tanggal_beasiswa">Tanggal beasiswa</label>
                                <input type="date" class="form-control" name="tanggal_beasiswa" id="tanggal_beasiswa" required value="<?= $mkn ["tanggal_beasiswa"]; ?>">
                            </div>
                        <div class="mb-3">
                            <label for="exampleInputNo" class="form-label" for="gambar_beasiswa"></label>
							<img class="gambar_beasiswa" src="../image/<?= $mkn ["gambar_beasiswa"]; ?>">
                            <input type="file" class="form-label" name="gambar_beasiswa" id="gambar_beasiswa">
                            </div>
                        <div class="text-center my-5"> 
                            <button type="submit" name="submit" class="fw-bold btn btn-custom2">Ubah Data beasiswa</button>
                            </div>
                        </form>
                    </div>
                </div>
